...
<div class="subject">
    ...
    <div class="box_B">
        ...
        <a href="SekillTime.jsp" class="btn_B" >时间段</a>
    </div>
    ...
    <script>
        layui.use('table',function () {
            ...
            table.render({
                elem:'#test',
                url:'${Spage}/admin/SekillServlet?type=seckill',
                type:'post',
                cellMinWidth:100,
                page:true,
                cols:[[
                    {field:'id',title:'编号',align:'center',width:80},
                    {field:"name",title: "活动标题",align:'center'},
                    {title: "活动状态",align:'center',width:120},
                    {title: "开始时间",align:'center',width:140,templet:function(d){
                            var date = new Date(d.openDate);
                            var YY = date.getFullYear() + '-';
                            var MM = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) + '-';
                            var DD = (date.getDate() < 10 ? '0' + (date.getDate()) : date.getDate());
                            return YY + MM + DD;
                        }},
                    {title: "结束时间",align:'center',width:140,templet:function(d){
                            var date = new Date(d.stopDate);
                            var YY = date.getFullYear() + '-';
                            var MM = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) + '-';
                            var DD = (date.getDate() < 10 ? '0' + (date.getDate()) : date.getDate());
                            return YY + MM + DD;
                        }},
                    {title: "上线/下线",align:'center',width:120,templet:function(d) {
        ...
        function stopDate(date) {
            var date = new Date(date.stopDate);
            var YY = date.getFullYear() + '-';
            var MM = (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) + '-';
            var DD = (date.getDate() < 10 ? '0' + (date.getDate()) : date.getDate());
            var hh = (date.getHours() < 10 ? '0' + date.getHours() : date.getHours()) + ':';
